...
class _TextFieldSelectionGestureDetectorBuilder extends TextSelectionGestureDetectorBuilder {
  _TextFieldSelectionGestureDetectorBuilder({required _RadixTextFieldState state})
    : _state = state,
      super(delegate: state);

  final _RadixTextFieldState _state;
...
///
/// To integrate the [RadixTextField] into a [Form] with other [FormField] widgets,
/// consider using [TextFormField].
///
/// {@template radix.textfield.wantKeepAlive}
...
///
/// {@tool dartpad}
/// This example shows how to create a [RadixTextField] that will obscure input.
/// The [RadixInputDecoration] surrounds the field in a border using [OutlineInputBorder].
///
/// ** See code in examples/api/lib/text_field/text_field.0.dart **
/// {@end-tool}
///
/// ## Reading values
///
/// A common way to read a value from a RadixTextField is to use the
/// [onSubmitted] callback. This callback is applied to the text field's
/// current value when the user finishes editing.
///
/// {@tool dartpad}
/// This sample shows how to get a value from a RadixTextField via the
/// [onSubmitted] callback.
///
/// ** See code in examples/api/lib/text_field/text_field.1.dart **
/// {@end-tool}
///
...
/// callback.
///
/// Keep in mind you can also always read the current string from a
/// RadixTextField's [TextEditingController] using [TextEditingController.text].
///
...
///
/// {@tool dartpad}
/// This sample shows how to style a text field to match a surface or soft
/// Radix text field.
///
/// ** See code in examples/api/lib/text_field/text_field.2.dart **
/// {@end-tool}
///
/// ## Scrolling Considerations
///
/// If this [RadixTextField] is not a descendant of [Scaffold] and is being used
...
/// [RadixTextField] to ensure proper scroll coordination for [RadixTextField] and its
...
///  * [InputDecorator], which shows the visual elements that surround the actual
///    text editing widget.
///  * [EditableText], which is the raw text editing control at the heart of a
///    [RadixTextField]. The [EditableText] widget is rarely used directly unless
...
class RadixTextField extends StatefulWidget {
  ...
  /// [RadixTextField.noMaxLength] then only the current length is displayed.
  ///
  ...
  const RadixTextField({
    ...
    this.focusNode,
    this.undoController,
    this.decoration = const RadixInputDecoration(),
    ...
    this.hintLocales,
       ...
       assert(maxLength == null || maxLength == RadixTextField.noMaxLength || maxLength > 0),
  ...
  /// this value.
  ///
  /// When this is not null, the field's height constraint will no longer
  /// be tightly bound by [RadixInputDecoration.contentHeight]. Instead,
  /// [RadixInputDecoration.contentHeight] will become the minimum height
  /// of the input field.
  ///
  ...
  /// to [RadixTextField.noMaxLength] then only the current character count is displayed.
  ///
  /// After [maxLength] characters have been input, additional input
  /// is ignored, unless [maxLengthEnforcement] is set to
  /// [MaxLengthEnforcement.none].
  ...
  /// which is evaluated after the supplied [inputFormatters], if any.
  ///
  /// This value must be either null, [RadixTextField.noMaxLength], or greater than 0.
  /// If null (the default) then there is no limit to the number of characters
  /// that can be entered. If set to [RadixTextField.noMaxLength], then no limit will
  ...
  ///
  /// The [mouseCursor] is the only property of [RadixTextField] that controls the
  ...
  static Widget _defaultContextMenuBuilder(
    ...
    if (!editableTextState.widget.readOnly && defaultTargetPlatform == TargetPlatform.iOS && SystemContextMenu.isSupported(context)) {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<RadixInputDecoration>(
        'decoration',
        decoration,
        defaultValue: const RadixInputDecoration(),
      ),
    );
    properties.add(
...
class _RadixTextFieldState extends State<RadixTextField>
  ...
  Color _getCursorErrorColor(ThemeData theme, RadixInputDecorationThemeData defaultDecoration) {
    return widget.decoration?.cursorErrorColor ??
        widget.decoration?.errorStyle?.color ??
        defaultDecoration.cursorErrorColor ??
        defaultDecoration.errorStyle?.color ??
  ...
  Color? _getSelectionColor(RadixInputDecorationThemeData defaultDecoration, DefaultSelectionStyle selectionStyle) {
    return widget.decoration?.selectionColor ??
        defaultDecoration.selectionColor ??
  ...
  RadixInputDecoration _getEffectiveDecoration(RadixInputDecorationThemeData defaults) {
  ...
  @override
  void didUpdateWidget(RadixTextField oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    bool? cursorOpacityAnimates = widget.decoration?.cursorOpacityAnimates ?? defaultDecoration.cursorOpacityAnimates;
    Offset? cursorOffset;
    final Color cursorColor;
    final Color selectionColor;
    Color? autocorrectionTextRectColor;
    Radius? cursorRadius = widget.decoration?.cursorRadius ?? defaultDecoration.cursorRadius;
    ...
    switch (theme.platform) {
      case TargetPlatform.iOS:
        ...
        cursorColor = _hasError
            ? _getCursorErrorColor(theme, defaultDecoration)
            : _getCursorColor(defaultDecoration, selectionStyle) ?? cupertinoTheme.primaryColor;
        selectionColor = 
            _getSelectionColor(defaultDecoration, selectionStyle) ?? cupertinoTheme.primaryColor.withOpacity(0.40);
      ...
      case TargetPlatform.fuchsia:
        ...
        cursorColor = _hasError
            ? _getCursorErrorColor(theme, defaultDecoration)
            : _getCursorColor(defaultDecoration, selectionStyle) ?? theme.colorScheme.primary;
        selectionColor =
            _getSelectionColor(defaultDecoration, selectionStyle) ?? theme.colorScheme.primary.withOpacity(0.40);
    ...
    Widget child = RepaintBoundary(
      child: UnmanagedRestorationScope(
        ...
        child: EditableText(
          ...
          mouseCursor: MouseCursor.defer, // RadixTextField will handle the cursor
          cursorWidth: widget.decoration?.cursorWidth ?? defaultDecoration.cursorWidth,
          cursorHeight: widget.decoration?.cursorHeight ?? defaultDecoration.cursorHeight,
    ...
    if (widget.decoration != null) {
      child = AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget? child) {
          return RadixInputDecorator(
            decoration: _getEffectiveDecoration(defaultDecoration),
            textAlign: widget.textAlign,
